...
# Configure caching for 2021
CACHE_DIR = os.path.join('cache', '2021')
...
CIRCUIT_CACHE_FILE = 'circuit_coordinates_2021.json'
    ...
    try:
        with pd.ExcelWriter(template_path, engine='openpyxl', mode='w') as writer:
            for race_key, race_info in races_data.items():
                # Prepend "2021_" to differentiate if needed.
                sheet_name = f"2021_{race_key}"
        ...
        print(f"2021 data saved successfully to {template_path}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    all_races = process_all_races(RACES_2021)
    ...
    TEMPLATE_PATH = os.path.join(TEMPLATE_DIR, 'F1_Seasons_2021.xlsx')
