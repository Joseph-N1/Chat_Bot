...
# Configure caching for 2020
CACHE_DIR = os.path.join('cache', '2020')
...
CIRCUIT_CACHE_FILE = 'circuit_coordinates_2020.json'
    ...
    try:
        with pd.ExcelWriter(template_path, engine='openpyxl', mode='w') as writer:
            for race_key, race_info in races_data.items():
                # Prepend "2020_" to differentiate if needed.
                sheet_name = f"2020_{race_key}"
        ...
        print(f"2020 data saved successfully to {template_path}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    all_races = process_all_races(RACES_2020)
    ...
    TEMPLATE_PATH = os.path.join(TEMPLATE_DIR, 'F1_Seasons_2020.xlsx')
