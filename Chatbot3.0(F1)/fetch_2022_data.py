...
# Configure caching for 2022
CACHE_DIR = os.path.join('cache', '2022')
...
CIRCUIT_CACHE_FILE = 'circuit_coordinates_2022.json'

# Define all 2022 races (year, round number, race name)
RACES_2022 = [
    ...
    try:
        with pd.ExcelWriter(template_path, engine='openpyxl', mode='w') as writer:
            for race_key, race_info in races_data.items():
                # Prepend "2022_" to differentiate if needed.
                sheet_name = f"2022_{race_key}"
        ...
        print(f"2022 data saved successfully to {template_path}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    all_races = process_all_races(RACES_2022)
    ...
    TEMPLATE_PATH = os.path.join(TEMPLATE_DIR, 'F1_Seasons_2022.xlsx')
